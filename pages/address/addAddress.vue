	...
	<div class="address_page">
		<section class="page_text_container">
			...
			<section class="section_list">
				...
				<section class="section_right">
					<div class="choose_address" @click="searchAdd">{{searchAddress? searchAddress.name : '小区/写字楼/学校等'}}</div>
	...
	export default {
		...
		methods: {
			...
			async addAddress() {
				...
				let res = await postAddAddress(this.userInfo.user_id, this.searchAddress.name, this.address_detail, this.geohash,
					this.name, this.phone, this.anntherPhoneNumber, 0, this.sex, this.tag, this.tag_type);
				if(res.status === 1){
					uni.navigateBack();
				}	
				else{
